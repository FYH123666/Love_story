...
CREATE TABLE IF NOT EXISTS `articles` (
  ...
  `edit_mode` enum('full','blocks','chat') NOT NULL DEFAULT 'full' COMMENT '编辑模式',
  `cover_image` varchar(500) DEFAULT NULL COMMENT '文章封面图',
  `cover_thumb_url` varchar(500) DEFAULT NULL COMMENT '封面缩略图(320px)',
  `cover_webp_url` varchar(500) DEFAULT NULL COMMENT '封面WebP',
  `created_at` datetime NOT NULL COMMENT '创建时间',
  `updated_at` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
...
CREATE TABLE IF NOT EXISTS `album_images` (
  ...
  `thumbnail_path` varchar(255) DEFAULT NULL COMMENT '缩略图路径(旧640px)',
  `thumb_url` varchar(500) DEFAULT NULL COMMENT '小缩略图URL(320px)',
  `medium_url` varchar(500) DEFAULT NULL COMMENT '中尺寸URL(1200px)',
  `webp_url` varchar(500) DEFAULT NULL COMMENT '主图WebP',
  `thumb_webp_url` varchar(500) DEFAULT NULL COMMENT '缩略图WebP(320px)',
  `medium_webp_url` varchar(500) DEFAULT NULL COMMENT '中尺寸WebP(1200px)',
...
('image_optimize_enabled', '1', '是否启用图片压缩优化'),
('image_thumb_enabled', '1', '多尺寸缩略图开关（320px/1200px）'),
('image_webp_enabled', '1', 'WebP自动生成开关'),
('image_lazy_enabled', '1', '前端懒加载开关'),
('image_cdn_url', '', 'CDN域名前缀（留空使用站点URL）');
